...
-- employees has same salary as that of their department .

select empid, empname ,deptid,salary from employees where 
(salary, deptid) in
(select avg(salary),deptid from employees group by deptid);


select e.empid, e.empname , e.deptid , e.salary from employees as e
where e.salary >
(select avg(salary) from employees as e2 where e.deptid = e2.deptid);
...
select s.studentid, s.studentname , s.department from students as s where ;
